const Pramanikshashtra = artifacts.require('./Pramanikshashtra.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('Pramanikshashtra', ([deployer, uploader]) => {
  let Pramanikshashtra

  before(async () => {
    Pramanikshashtra = await Pramanikshashtra.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await Pramanikshashtra.address
    ...
    it('has a name', async () => {
      const name = await Pramanikshashtra.name()
      assert.equal(name, 'Pramanikshashtra')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await Pramanikshashtra.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await Pramanikshashtra.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await Pramanikshashtra.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await Pramanikshashtra.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await Pramanikshashtra.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await Pramanikshashtra.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await Pramanikshashtra.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await Pramanikshashtra.files(fileCount)
